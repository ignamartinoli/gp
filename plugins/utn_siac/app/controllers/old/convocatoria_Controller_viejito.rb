...
class ConvocatoriaController < ApplicationController
 ...
 def cargar_especialidades
  ...
  def index
    mostrar_cerradas = (params[:mostrar_cerradas] == "true")

    require "date"

    # Parámetros de la función
    solo_activas = !mostrar_cerradas
    estado_actual = nil
    fecha_desde = nil
    fecha_hasta = nil
    ...
    rows = Siac::SiacRepository.function_typed(
      ...
      [
        { value: solo_activas, cast: "boolean" },
        { value: estado_actual, cast: "int" },
        { value: fecha_desde, cast: "date" },
        { value: fecha_hasta, cast: "date" }
      ]
    )
    ...
    dtos = rows.map do |r|
      # Mapeo a los nombres que tu vista usa
    ...
    # Orden: como no hay bookmarks, ordenamos por fecha desc.
